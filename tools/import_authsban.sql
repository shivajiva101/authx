...
Use this file to import the data set from auth.sqlite and sban.sqlite
database files by copying them to the world folder you want to apply
it to and importing it with sqlite from that location.
...
INSERT INTO auth_tmp (id, name, password, last_login)
	SELECT name.id, name, password, last_login from auth.auth
	INNER JOIN sban.name ON name = auth.name;
...
-- copy data
INSERT OR REPLACE INTO auth (id, name, password, privileges, last_login)
	SELECT * from auth_tmp;

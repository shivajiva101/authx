...
-- Incrememnts an integer
-- @param int integer input
-- @return integer incremented by 1
...
-- @param name_or_ip string
-- @return nil
local function add_blacklist_record(source, name_or_ip, reason)
	local ts = os.time()
	...
	local stmt = ([[
			...
			VALUES ('%s', '%s', '%s', %i)
	]]):format(name_or_ip, reason, source, ts)
	return db_exec(stmt)
end
...
local function create_ban_record(name, source, reason, expires)
	...
	if expires ~= 0 then
		...
		msg_l = ("[authx] %s temp banned by %s reason: %s"
		):format(name, source, reason)
	else
		msg_k = ("Banned: Reason: %s"):format(reason)
		msg_l = ("[authx] %s banned by %s reason: %s"
		):format(name, source, reason)
	end
...
-- Create a setting with data
-- @param setting string
-- @param data string(serialised)
-- @return nil
...
local function auth_add_record(name, password, privs, last_login)
	local ts = os.time()
	auth_cache[name] = {
		password = password,
		privs = privs,
	...
	]]):format(name, password, privs, last_login, 0, ts)
	return db_exec(stmt)
end
...
local function update_ban(id, source, reason, name)
	...
	-- log event
	minetest.log("action",
	("[authx] %s unbanned by %s reason: %s"):format(name, source, reason))
end

-- Update violation status
-- @param ip string
-- @return result of db_exec function
...
authx.auth_handler = {
	get_auth = function(name, add_to_cache)
		...
		if name == nil or name == '' or name == ' ' then
			minetest.log("info", "[authx] Name missing in call to get_auth. Rejected.")
			return nil
		end
	...
	create_auth = function(name, password)
		...
		auth_add_record(name,password,privs,ts)
		return true
	end,
		...
		for i, v in ipairs(data) do
			...
			if ip and name then
				...
				if not bans[id] then
					...
					create_ban_record(name, 'authx', 'imported from ipban.txt', '')
				end
			end
		end
	end
	...
	local function db_import()
		...
		if result then
			if not get_setting("auth_import") then
				init_setting("auth_import", 'true') -- set db flag
			end
		end
...
local function clean_join_cache(name)
	...
	for k,v in pairs(t_id) do
		if v.ts and (v.ts + TTL) < ts or k == name then
			t_id[k] = nil
		end
	end
end
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if not privs.ban then
		...
		create_ban_record(name, 'authx',
		'detected using a hacked client to access the ban GUI!')
		return
	end
	...
	func = function(name, params)
		...
		if id then
		   ...
		   if bans[id] then
			...
			add_blacklist_record(name, player_name, reason)
		end
...
minetest.override_chatcommand("unban", {
	...
	func = function(name, params)
		...
		if id then
			...
			update_ban(id, name, reason, name_or_ip)
			return true, ("Unbanned %s."):format(name_or_ip)
		else -- check blacklist
			local key = name_or_ip
			if is_ip(key) then key = ip_key(key) end
			if BL[key] then
				del_blacklist_record(name_or_ip)
				return true, ("Unbanned %s."):format(name_or_ip)
			end
		end
	end,
...
minetest.register_on_joinplayer(function(player)
	...
	if id then -- known name?
		-- check id records match current ip
		local target_id = get_id(ip)
		if not target_id then -- unknown ip
			add_ip(id, ip) -- new ip record
		elseif id ~= target_id then -- id violation
			-- IP is registered to another id! Log event
			manage_idv_record(id, target_id, ip)
			update_idv_status(ip)
		else -- known name & ip for id
